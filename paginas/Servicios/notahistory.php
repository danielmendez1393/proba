...
if( $pag <> 1) $inicio = ($pag*5)+1;
else $inicio = 0;

//Consultar en esta página los anuncios 
require_once($CONFIG['pathinclude']."cls/notas.php");
$objNotas = new notas;
$totalregistros = $objNotas->NumNotasTotal($_GET['mes']?$_GET['mes']:0,$_GET['anio']?$_GET['anio']:0,$_GET['texto']);
if( strlen($_GET['texto'])>0 ){
	$notas = $objNotas->NotasBuscar($_GET['texto'],$inicio);
}else{
	$notas = $objNotas->NotasHistorial($_GET['mes'],$_GET['anio'],$inicio); 
}
$paginas = round($totalregistros/5);
//print_r($nota);
unset($objNotas);
                ...
                <div class="container bgwhite">
                    ...
                    <div class="row">
                       ...
                       <div class="col-md-8 col-sm-7 col-xs-12">
                           <?php
						  if(strlen($_GET['texto'])>0) echo "<p>Resultados encontrados con el texto: ".$_GET['texto']."</p>";
						   foreach($notas as $id => $nota){
                           ?>
                            <div class="cv-theme-pa-30">
                                <div class="standard-post-thumbnail">
                                    <div class="post-image-block">
                                        <a href="blog_signal.html"><img src="<?=$CONFIG['sitioimgnotas'].$nota['nts_imagen']?>" alt="" class="img-responsive"></a>
                                    </div>
                                    <div class="blog-dis">
                                        <h2 class="entry-title"><a href="<?=$CONFIG['sitio']?>paginas/Servicios/notadetalle.php?nts_id=<?=$nota['nts_id']?>"><?=$nota['nts_titulo']?></a></h2>
                                        <p class="post-meta">Por <span class="author vcard"><a title="Posts By Admin" href="#">Difusion</a></span>  |  <span class="published"><?=$nota['nts_fecha']?></span>  </p>
                                        <p><?=$nota['nts_descripcioncorta']?>.... </p>
                                    </div>
                                </div>
                            </div>
                            <?php
                            ...
                            <div class="cd-pagination">
                                <ul class="pagination">
                                    ...
                                    if($pag>1){
									?>
                                    <li><a href="#" onClick="javascript: changePag('<?=$pag-1?>')"><i class="fa fa-angle-left" aria-hidden="true"></i></a></li> 
                                    <?php
									}
                                    for($i=1; $i < $paginas; $i++ ){
									?>
                                    <li <?php if($pag == $i){ ?> class="active"<?php }?>><a href="#" onClick="javascript: changePag('<?=$i?>')"><?=$i?></a></li>
                                    <?php
									}
									 if($pag+1 < $paginas){
									?>
                                    <li><a href="#" onClick="javascript: changePag('<?=$pag+1?>')"><i class="fa fa-angle-right" aria-hidden="true"></i></a></li>
                                    <?php
									 }
									?>
                                </ul>
